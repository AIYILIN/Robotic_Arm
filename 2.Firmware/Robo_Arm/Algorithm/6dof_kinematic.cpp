...
bool DOF6Kinematic::SolveFK(const DOF6Kinematic::Joint6D_t &_inputJoint6D, DOF6Kinematic::Pose6D_t &_outputPose6D)
{
    ...
    snprintf(dbg_msg, sizeof(dbg_msg),
             ...
             "L0_ew=%.3f,%.3f,%.3f\n"
             "L0_wt=%.3f,%.3f,%.3f\n",
             L0_bs[0], L0_bs[1], L0_bs[2],
             L0_se[0], L0_se[1], L0_se[2],
             L0_ew[0], L0_ew[1], L0_ew[2],
             L0_wt[0], L0_wt[1], L0_wt[2]);
    HAL_UART_Transmit(&huart1, (uint8_t *)dbg_msg, strlen(dbg_msg), 100);
    ...
    // // 在计算旋转矩阵后添加调试输出
    // if (R06 != nullptr) {  // 假设R06是计算得到的旋转矩阵
    //     snprintf(dbg_msg, sizeof(dbg_msg),
    //         "[R06 Matrix]\r\n"
    //         "%.4f  %.4f  %.4f\r\n"
    //         "%.4f  %.4f  %.4f\r\n"
    ...
    // // 打印最终计算结果
    // snprintf(dbg_msg, sizeof(dbg_msg),
    //     "[FK Output] X=%.3f Y=%.3f Z=%.3f\r\n",
    //     _outputPose6D.X, _outputPose6D.Y, _outputPose6D.Z);
    // HAL_UART_Transmit(&huart1, (uint8_t*)dbg_msg, strlen(dbg_msg), 100);
